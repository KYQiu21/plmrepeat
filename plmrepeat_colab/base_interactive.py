...
class RepeatDetectorInteractive:
    """
    ...
    def __init__(self,
        ...
        # first calculate all candidate repeat lengths
        # if self.ssa_result is not None:
        #     self._get_len_list()
    ...
    def get_len_list(self, repeat_class_idx=0):
    ...
    def search_one_len(self, estimate_repeat_length, repeat_class_idx=0):
        ...
        if self.draw_during_procedure:
            save_draw_repeat_path = f'{self.out_dir}/repeat_on_score_matrix{repeat_class_idx}.png'
            draw_repeat_matrix(self.score_matrix, (max_win_start, max_win_end), save=True, save_path=save_draw_repeat_path)
        ...
        if extract_results.shape[0] >= 1 and self.draw_during_procedure:
            save_draw_extract_repeat_path = f'{self.out_dir}/extract_repeat{repeat_class_idx}.png'
            draw_extract_repeat(extract_results, len(self.seq), estimate_repeat_length, save=True, save_path=save_draw_extract_repeat_path)
    ...
    def define_boundary(self, estimate_repeat_length, max_win_start, max_win_end, weight_repeat_emb, origin_repeat_emb):
        ...
        if have_overlap:
            ...
            self.single_result_info_dict = result_info_dict
    ...
    def report_one_len(self, estimate_repeat_length):
        """
        function to execute a single search on repeats - perform search_one_len for all repeat length in the list and store relevant information

        """

        max_win_start, max_win_end, weight_repeat_emb, origin_repeat_emb = self.search_one_len(estimate_repeat_length)
        # only go ahead if scan_repeat generates positive hits
        if self.extract_results.shape[0] > 1:
            self.define_boundary(estimate_repeat_length, max_win_start, max_win_end, weight_repeat_emb, origin_repeat_emb)
...
class AllRepeatDectorInteractive(RepeatDetectorInteractive):
    """
